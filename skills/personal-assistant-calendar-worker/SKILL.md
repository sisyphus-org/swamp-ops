...
The tool validates the Personal Assistant profile, pinned board, task/run/claim binding, live lease, and owner-attested clean runtime revision before workflow access. For a new explicit-intent write it performs plan, PII-free target-state snapshot, approval-workflow attestation, fresh pre-apply snapshot comparison, provider-atomic ETag preconditions, apply, and verified read-back in one run. Legacy preview approval tasks remain accepted for already-issued opaque references. It also owns bounded reads, replay handling, redaction, and terminal `calendar-result.v1` persistence. A per-command execution lock serializes load→execute→journal, and verified results are journaled atomically before Kanban completion so retry never repeats already-completed external work.
