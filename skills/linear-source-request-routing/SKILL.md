...
- Archive one exact issue (`{identifier:SIS-N}`), unique SIS-scoped project (`{name}`), or unique initiative (`{name}`) with the fixed approval reference. The retained direct fixed-approved `delete_linear_entity` matrix accepts an exact issue, SIS-scoped project, project-scoped milestone, or initiative with that same fixed approval object. Separately, an approval-less direct `delete_linear_entity` request is generated-preview-only and issue-only: PM reads the exact current issue/project/milestone and complete children/relation impact without mutation or recovery-journal writes, then source returns an opaque 15-minute approval reference. Only `approve_delete_linear_entity` with that same-session reference may queue the bound issue trash delete. Caller-supplied booleans, hashes, or attestation objects cannot approve the generated-preview flow.
- Route `bulk_linear_operations` with an ordered 1–50 `items` list. Each item is exactly `{operation,target,change}` in an already-supported mutating lane shape. Never add child approval/policy/IDs. Safe-only batches execute directly. An unapproved batch containing an owner-controlled child returns a 15-minute ordered safe preview and opaque `linear-bulk-approval:v1` reference; after explicit confirmation call owner-only broker `approve_linear_bulk_preview`, then source `approve_bulk_linear_operations` with only that same-session reference. Source reconstructs the exact original ordered batch and approval internally. Never supply hashes, policy, selectors, caller identity, or attestation fields to either confirmation call.
...
2. Call `linear_source_request` once with the matching bounded shape:
   ...
   - issue move: `operation=move_issue`, exactly one issue target, exact current `expected_project` plus nullable `expected_milestone`, and exact target `project`/`milestone` names. Both expected fields may be `null` for an unscoped issue; a named milestone without a named project is invalid. Resolve all names first. Never attach approval: this is a standard compare-before-set move through the existing fixed issue writer;
   ...
   - standalone: `operation=create_standalone_issue` with `project={"name":"<exact existing project>"}` and `milestone={"name":"<exact existing milestone>"}` objects plus an `issue` containing exact `title`, `description`, safe `state`, and `priority`. Both scope objects are mandatory; never pass a bare string or `null`. If the owner names only a project, search its milestones and ask one focused question only when more than one plausible target remains;
   - issue tree: `operation=converge_issue_tree` with the same mandatory project/milestone object fields plus `sub_issues` containing 1–10 exact issue objects. Omit optional sub-issues from this list if they must remain uncreated.
   ...
   - archive: `operation=archive_linear_entity`, exact singular supported `entity_type`, exact typed `selector`, and the existing fixed `approval`;
   - retained direct fixed-approved delete: `operation=delete_linear_entity`, exact singular `entity_type` from `issue|project|milestone|initiative`, its exact typed selector, and the existing fixed `approval` object. This compatibility path does not generate a preview;
   - generated issue-delete preview: first call `operation=delete_linear_entity`, `entity_type=issue`, and exact selector `{identifier:SIS-N}` with no approval fields. Report the returned target/context, children/relations impact, 30-day recoverable-trash semantics, expiry, and opaque reference. After the owner explicitly confirms that preview, call `ops_broker` with a newly generated UUID `request_id`, `integration=swamp`, `operation=approve_linear_delete_preview`, `mode=apply`, and `arguments={"approval_reference":"<same opaque reference>"}`. This operations request is asynchronous: after `queued`, stop; on the exact-session Kanban wake, replay the literal same five-field `ops_broker` request once. Only after its sanitized completed result reports `ready=true` call `linear_source_request` with `operation=approve_delete_linear_entity` and only the same reference. The reference is consumed before Swamp starts. If the completed operations result reports an unknown approval outcome, do not retry that reference: repeat the approval-less issue delete to reserve/reuse one fresh trusted preview successor and ask for confirmation again. Never accept or construct `approved`, `before_state_hash`, policy, or attestation fields at source.
   - ordered batch: call `bulk_linear_operations` with exact ordered `items`. Safe-only batches queue directly. Owner-controlled batches without `approval` return `phase=awaiting_approval`; after explicit confirmation call `ops_broker` for `approve_linear_bulk_preview` with only its opaque reference. That operations request is asynchronous: after `queued`, stop; on the exact-session wake, replay the literal same five-field `ops_broker` request once. Only after its sanitized completed result reports `ready=true` call source `approve_bulk_linear_operations` with only the same reference. Preserve order literally; never resend items or supply hash/policy/selector/caller identity on confirmation. The retained fixed-approved compatibility form may carry one parent `approval`.
...
- Source accepts no arbitrary `policy`, approval boolean, before-state hash, path, manifest, shell text, raw relation ID, or caller-selected workflow/model. Delete confirmation accepts only the opaque same-session reference and loads the broker-granted policy from protected Kanban state; the model never supplies or receives its attestation internals. Other owner-controlled shapes retain the fixed `approval` object and emit exactly `{mode: owner_approved, approval: <reference>}`. Policy is part of replay identity.
