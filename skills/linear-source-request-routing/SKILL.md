...
2. Call `linear_source_request` once with the matching bounded shape:
   ...
   - issue tree: `operation=converge_issue_tree` with the same mandatory project/milestone object fields plus `sub_issues` containing 1–10 exact issue objects. Omit optional works from this list if they must remain uncreated.
