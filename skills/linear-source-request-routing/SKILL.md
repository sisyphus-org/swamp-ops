...
2. Call `linear_source_request` once with the matching bounded shape:
   ...
   - ordered batch: `operation=bulk_linear_operations`, `items=[...]`, and optionally the one parent `approval` only when at least one item requires it. Preserve item order literally; reject duplicates and repeated exact targets instead of trying to merge them.
