...
6. On the later Kanban wake, read `linear-result.v1` and send one concise response with the canonical Linear URL. Do not repeat raw Kanban lifecycle text.
...
- One semantic idempotency key maps to one active task and one verified external result.
