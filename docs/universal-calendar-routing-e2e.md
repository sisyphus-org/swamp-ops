...
- Explicit-intent write: a clear authenticated owner instruction authorizes one exact bounded create, update, or delete on the protected profile-local Calendar target; every new closed plan binds that target only by domain-separated `calendarTargetHash`, and apply plus the worker reload the protected target to verify it without placing the identifier in plan stdout/logs. The worker executes plan → snapshot → attestation → fresh snapshot → apply → read-back in one run without a second conversational confirmation. Standalone events omit `linear_url`; an explicitly requested canonical public Linear issue URL remains optional and is preserved in linked descriptions. The only plan-format compatibility exception is a closed legacy `calendarId: "primary"` artifact while the current target is OAuth `primary`; service-account mode rejects it.
