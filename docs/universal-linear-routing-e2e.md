...
If both immediate deterministic create attempts encounter an allowlisted transient provider failure and exact read-back proves the issue absent after each attempt, PM emits a typed verified-absence blocker rather than a generic backend error. On the first such blocked delivery, the source route re-audits the exact wake binding and requeues that same task once. A second verified-absence block is terminal and exposes only the factual allowlisted reason. No new mutation key, task, title-based attribution, or unconstrained retry is created.
