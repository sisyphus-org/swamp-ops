...
- `bulk_linear_operations` for 1–50 ordered exact mutating items, with all-child preflight before write, one aggregate owner approval when needed, deterministic child identities, fsync-backed ordered resume, and safe outcomes/counts-only source projection. Nested bulk, reads, bulk selectors, duplicate items, and repeated exact targets are rejected.
