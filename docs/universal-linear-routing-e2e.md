...
- supports bounded comment, direct standard transition from any current state to all seven directly writable states, compare-before-set movement of an existing issue between exact project/milestone scopes, reserved `Duplicate` through an exact duplicate-to-canonical relation, issue-number targeting inside the single `SIS` team, deterministic description link removal, issue creation under an exact `SIS-N` parent, create-only hierarchy convergence, standalone top-level creation in an exact existing project/milestone, one top-level issue plus 1–10 explicit sub-issues, and non-destructive exact-name initiative create/update/project-link operations without caller-controlled entity IDs and with exact read-back;
...
The paired `linear-source-request-routing` skill requires short factual replies: `Принято, выполняю.` while queued, then only the final user-visible result and canonical Linear URL. It forbids Kanban inspection after queueing, direct Linear mutation, generic GraphQL, terminal fallback, passive lifecycle narration, invented Telegram topics, or another profile's bot. For new `create_standalone_issue` and `converge_issue_tree` calls, `project` and `milestone` are exact existing-scope selectors and the model is instructed to send only `{"name":"..."}`. The published schema retains optional descriptions solely for literal replay compatibility and labels them as legacy or owner-requested constraints. An allowlisted `project|milestone supplied description conflicts with live state` blocker additionally returns a bounded `recovery` instruction naming the exact optional field to omit when that description was not part of the owner's intent.
...
- `update_issue` for an explicit non-empty subset of `title`, literal `description` or `description_transform=remove_links`, safe `state`, bounded `priority`, `assignee`, exact `labels`, `due_date`, `estimate`, and `parent_identifier` on one exact `SIS-N`; project/milestone scope changes are rejected and exclusive to `move_issue`. The source accepts a positive `issue_number`, binds it to the fixed `SIS` team, and canonicalizes it to the exact `SIS-N` target before emitting the PM command;
- `move_issue` for one exact issue with expected current project/milestone names and one unique exact target project/milestone. It reuses the fixed issue update mutation, performs a final complete issue read and re-resolves the target by ID, exact name, SIS ownership, and containment immediately before write, rejects rename/replacement/new ambiguity before mutation, and includes URL and archived state in exact unmanaged-field read-back. It is valid inside crash-recoverable ordered bulk migration plans. Linear has no conditional revision input, so this does not claim atomic provider CAS;
- `add_comment` through the explicit structured source shape (`operation`, exactly one of `identifier`/`issue_number`, and `body`) with deterministic replay protection and a clean user-authored body. The legacy exact Russian request string is replay-compatible only and is not the source-agent API;
...
- `create_standalone_issue` for one top-level issue in an exact existing project/milestone, with explicit priority and absent parent; a unique exact-title legacy partial write may be safely adopted only inside the verified scope. Retryable transport/provider failures are reconciled first by deterministic issue ID: a landed write completes without a second mutation, while a verified-absent result permits one immediate retry with the same ID.
- `converge_issue_tree` for one top-level issue plus 1–10 explicit sub-issues, each with deterministic identity, exact parent read-back, crash recovery, and literal replay no-op. Parent and child creates use the same landed-write reconciliation and verified-absent bounded retry.
...
If both immediate deterministic create attempts encounter an allowlisted transient provider failure and exact read-back proves the issue absent after each attempt, PM emits a typed verified-absence blocker rather than a generic backend error. Retry authority comes only from `LinearProviderError` metadata preserved at the client boundary: explicit transient HTTP status, transport/malformed-response classification, or the exact GraphQL extension code `RATELIMITED`. Human-readable error words never authorize a retry. On the first such blocked delivery, the source route re-audits the exact wake binding and requeues that same task once. A second verified-absence block is terminal and exposes only the factual allowlisted reason. No new mutation key, task, title-based attribution, or unconstrained retry is created.
