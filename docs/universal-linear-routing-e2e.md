...
Comments, created issues, and newly created hierarchy entities use domain-separated deterministic Linear UUIDv4 IDs derived inside Project Manager from the global mutation key. For hierarchy requests, the unique exact team key `SIS` identifies the team independently of its mutable display name. Project and milestone lookup checks the candidate deterministic ID first, then may reuse one unique exact-name live entity after strict team-ID scope/project and supplied-description verification; issue identity never falls back to title. The PM journal request hash uses the same global payload and excludes source profile, command/correlation IDs, session, and delivery metadata. Therefore identical requests from different profiles or sessions receive distinct exact-wake tasks but share the PM mutation key and converge to one global apply plus verified no-ops. `source_profile` remains in each run's result/audit. Public body/description stays exactly user-authored. Hierarchy results return the real IDs of reused project/milestone entities and the deterministic issue ID; no operation sets `linear-result.v2.verified=true` until exact scoped read-back matches every supplied scalar and structural field.
