...
```json
{
  ...
  "source_profile": "swe",
  "operation": "read_issue | search_linear | inventory_linear | change_state | move_issue | update_issue | add_comment | create_issue | create_issue_relation | remove_issue_relation | replace_issue_relation | ...",
...
`{"mode":"standard"}` remains the default and is used for every directly writable workflow-state transition and for exact duplicate relation creation. The existing exact fixed-shape `owner_approved` attestation reference documented in [linear-owner-approval.md](linear-owner-approval.md) is accepted only for parent-only `update_issue`, exact issue-relation removal/replacement, the supported core lifecycle matrix, and a `bulk_linear_operations` parent containing at least one such owner-controlled child. Owner-controlled children always reject standard policy.
...
- `change_state`: `change` must contain only `state`; standard exact allowlist is `Backlog`, `Todo`, `Research`, `In Progress`, `In Review`, `Done`, and `Canceled`. Every directly writable destination is reachable from every current state. `Done` and `Canceled` resolve uniquely in the target SIS team with live semantic types `completed` and `canceled`. Reserved `Duplicate` is not writable through `stateId`.
- `update_issue`: `change` is a non-empty subset of title, literal `description` or `description_transform=remove_links`, safe `state`, `High|Medium|Low` priority, assignee, exact labels, due date, estimate, and parent for one exact `SIS-N`. Project/milestone fields are rejected and must use `move_issue`. Literal description and transform are mutually exclusive. `remove_links` reads the live description, preserves visible Markdown labels and remaining text, removes HTTP(S) destinations, then applies all requested fields in one mutation with exact read-back; literal replay is a verified no-op.
- `move_issue`: moves one exact existing `SIS-N` through the same fixed issue writer while requiring an exact current `expected_project` with nullable `expected_milestone` (or both `null` for an unscoped issue) and exact target `project`/`milestone` names. A named expected milestone without a named expected project is invalid. PM re-reads and compares the complete issue, then re-resolves a unique target project/milestone by ID, exact name, SIS ownership, and containment immediately before Linear's non-conditional `issueUpdate`; rename, replacement, or newly introduced exact-name ambiguity blocks before mutation. Exact read-back includes URL and archived state as unmanaged fields. The move preserves identity/history/state/parent and every unmanaged field, and accepts exact already-converged replay as a verified no-op. The public Linear API exposes no atomic revision precondition, so this is compare-before-set with a minimal residual provider TOCTOU window, not an atomic CAS guarantee.
- `create_issue_relation`: safely creates one exact `blocks`, `blocked_by`, symmetric `related`, or directed `duplicate` relation between same-team non-self `SIS-N` issues. For `duplicate`, the target issue is marked as a duplicate of `related_identifier`; PM verifies the created relation and Linear's reserved `Duplicate` state.
...
- `add_comment`: source ingress uses structured `operation=add_comment`, exactly one exact `identifier` or positive `issue_number`, and `body`; PM receives `change` containing only `body`, 1–4000 characters. The public comment body remains exactly this user-authored text; credential-shaped data and reserved internal marker text cannot be supplied by the caller. The old exact Russian source string remains accepted only for backward-compatible replay. Any stale caller that submits a fresh comment through the legacy `request` field—whether or not its prose matches the former exact grammar—receives bounded, actionable guidance naming the structured fields instead of a generic rejection, while no task or mutation is created.
...
- `delete_linear_entity`: the retained direct fixed-approved path supports issue, project, project-scoped milestone, or initiative. Issue deletion is non-permanent trash semantics. Every lifecycle plan binds the exact identity-bearing entity and complete impact inventory, and apply re-plans before entering the write path and compares its own final full inventory immediately before journaling/mutation.
- `preview_delete_linear_entity`: additional trusted read-only issue-only delete preflight, generated only from an approval-less exact issue request. It resolves exactly one active `SIS-N`, returns the safe projection of the complete before/impact plan and a 15-minute expiry, performs no mutation, and creates no recovery journal. Public source projection replaces the internal identity-bearing before-state hash with an opaque owner/session/target-bound approval reference.
- `preview_bulk_linear_operations`: internal read-only form generated for an unapproved batch containing an owner-controlled child. It invokes the existing child planners in exact order through a module-private plan-only capability, aggregates each trusted child before-state commitment (including lifecycle identity hashes), expires after 15 minutes, and has no apply or journal authority. Public confirmation surfaces are only `approve_linear_bulk_preview` at broker and `approve_bulk_linear_operations` at source, both carrying the opaque same-session reference alone.
...
- Child, hierarchy, standalone, issue-tree, and exact issue-update paths share one comparison module. Mutation text is never rewritten. Read-back first accepts exact bytes, then only a whole-value byte-exact match to one independently confirmed Linear serialization: every unambiguous plain HTTP(S) URL becomes `[url](<url>)`, or every eligible Markdown unordered-list marker changes from `- ` to `* `. List-marker equivalence is anchored to complete list-item lines, supports nested items only below a prior list item, and excludes task lists, fenced or ambiguous indented code, thematic breaks, `+` markers, partial conversion, changed text/markup, and whitespace drift. URL and list transformations are not composed without separate production evidence.
...
Closure to `Done`, reopening from it, and all directly writable workflow-state transitions use the normal exact standard-policy route. The reserved `Duplicate` outcome uses the exact duplicate relation route.
