...
- **Read-back/replay**: create inserts only when absent; update requires the linked target and uses `events.update`; delete requires the linked target and must read back a 404. Exact create/update replay and already-absent delete are verified no-ops. Ambiguous write responses are reconciled by deterministic GET.
