...
SIS-60 keeps Kanban execution internal and returns the user-facing result through the existing Telegram DM session thread of the originating profile. Creating a new topic per Kanban task is not part of the contract. The broker owns dispatch only; it never owns Telegram delivery.

## Contract

For a task created from an existing Telegram DM session thread, the source profile must:
...
2. Keep the task and raw terminal event internal; do not create a new per-task Telegram topic or send a passive Kanban status ping.
3. Upsert exactly one notification subscription with:
   - `platform=telegram`;
   - exact source `chat_id`, `user_id` and existing numeric `thread_id`;
   - `chat_type=dm`;
   - `notifier_profile=<source profile>`;
   - `delivery_mode=wake`;
   - delivery metadata either absent or exactly `{"chat_type":"dm"}`; no created-topic/reply/fallback fields.
...
`notify-subscribe` uses the existing session-thread subscription key and updates the route rather than creating a second route. Source profiles must complete this step and require an audit pass while the task remains in triage. A fully typed task is then released through the shipped `specify_triage_task` kernel transition with its existing title/body/assignee unchanged; that atomically moves `triage → todo` and runs `recompute_ready`, so a parent-free task must reach `ready`. Do not call `promote_task` directly on `triage`: it accepts only `todo` or `blocked`. A setup failure leaves the task in triage for reconciliation; it must not be delivered by broker, default or another profile.
...
Healthy output has `result=pass`, `readOnly=true`, the exact source profile/chat/user/thread route, the matching persisted source `session_id`, `chat_type=dm`, `delivery_mode=wake`, metadata absent or exactly `{"chat_type":"dm"}`, and no `pending_terminal_events`. The audit rejects broker ownership, wrong/missing thread, created-topic/reply/fallback metadata, passive notification modes, wrong participant or session identity, duplicate subscriptions, malformed identifiers, path traversal and unconsumed terminal events.
...
- The existing SWE session thread is reused; no new `/topic` is created for an internal task.
...
2. Upsert the exact existing session-thread wake-only subscription.
...
6. Confirm the source profile gateway logs `kanban notifier: woke agent for <task>` and the exact thread-bound Hermes session is reused.
...
3. Verify the subscription cursor is unchanged and no other profile/thread route delivers the event.
4. Restart the source Gateway.
5. Verify wake-only delivery retries in the exact session thread, the source agent replies once, and the cursor advances only after successful wake.
