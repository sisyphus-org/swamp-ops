...
def main() -> int:
    ...
    rendered = render_config(provider, default_model, workspace)
    ...
    shared_env_issues = []
    if not shared_linear_present:
        shared_env_issues.append(f"LINEAR_TOKEN is missing from {SHARED_ENV}")
    if not shared_telegram_allowlist_present:
    ...
    planned = {
        ...
        "telegramAllowlist": {
            ...
            "sharedValuePresent": shared_telegram_allowlist_present,
            "profileOverrideSupported": True,
            "profileValueRequired": False,
        },
        "requiredSharedEnv": SHARED_ENV_VARS,
        "requiredProfileEnv": PROFILE_ENV_VARS,
        "optionalProfileEnv": OPTIONAL_PROFILE_ENV_VARS,
        "ownerStepsBeforeActivation": [
            f"create {profile_dir / '.env'} with profile-specific variables only",
            "chmod 600 the profile .env",
            "authenticate the profile's model provider without copying auth files",
            "review and install a dedicated system LaunchDaemon",
            "start the gateway and run model, Russian STT, Linear MCP, and Telegram smoke tests",
        ],
    }
