...
SIS-123 routes bounded Calendar inventory/events/freebusy and explicit-intent create/update/delete from every general profile through the same exact-session Kanban bus to the headless Personal Assistant. The source and broker have no Google credentials/client; Personal Assistant has no Linear credentials/client. A clear authenticated owner write command authorizes that exact bounded mutation without a second conversational confirmation. The worker retains protected planning, a PII-free target-state snapshot, internal checksum-bound attestation, a fresh pre-apply snapshot, Google ETag `If-Match` for update/delete/restore, and verified read-back. Legacy pending previews remain same-session approval-gated. Deterministic event identity and verified read-back make replay a no-op. Calendar and Linear are independent: standalone events require no SIS issue or URL, while explicitly linked events preserve the canonical public Linear URL. Protocol and rollout gates are documented in [`docs/universal-calendar-routing-e2e.md`](docs/universal-calendar-routing-e2e.md).
